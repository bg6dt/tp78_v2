...
const uint32_t Row_Pin[ROW_SIZE] = { GPIO_Pin_0, GPIO_Pin_1, GPIO_Pin_9, GPIO_Pin_8, GPIO_Pin_7 };   //row 5 - 其它键盘布局需修改此处
const uint32_t Colum_Pin[COL_SIZE] = { GPIO_Pin_18, GPIO_Pin_19, GPIO_Pin_20, GPIO_Pin_8, GPIO_Pin_9, GPIO_Pin_17 };   //colum 6 - 其它键盘布局需修改此处
//row*colum = 6*14 = 84
const uint8_t KeyArrary[ROW_SIZE][COL_SIZE] = {
    ...
    /* ROW0 */ { KEY_GraveAccent, KEY_1, KEY_2, KEY_3, KEY_4, KEY_5 },
    /* ROW1 */ { KEY_TAB, KEY_Q, KEY_W, KEY_E, KEY_R, KEY_T },
    /* ROW2 */ { KEY_CapsLock, KEY_A, KEY_S, KEY_D, KEY_F, KEY_G },
    /* ROW3 */ { KEY_LeftShift, KEY_Z, KEY_X, KEY_C, KEY_V, KEY_B },
    /* ROW4 */ { KEY_LeftCTRL, KEY_LeftGUI, KEY_Delete, KEY_LeftAlt, KEY_None, KEY_SPACEBAR },
...
const uint8_t Extra_KeyArrary[ROW_SIZE][COL_SIZE] = {
    //-- COL 0 --|--- COL 1 ---|--- COL 2 ---|---- COL 3 ----|--- COL 4 ---|--- COL 5 ---|- COL 6 -|- COL 7 -|- COL 8 -|- COL 9 -|---- COL10 ----|- COL11 -|- COL12 -|-- COL13 --|
    /* ROW0 */ { KEY_None, KEY_None, KEY_None, KEY_None, KEY_None, KEY_None },
    /* ROW1 */ { KEY_None, KEY_None, KEY_None, KEY_None, KEY_None, KEY_None },
    /* ROW2 */ { KEY_None, KEY_BACKSPACE, KEY_UpArrow, KEY_ENTER, KEY_PageUp, KEY_None },
    /* ROW3 */ { KEY_None, KEY_LeftArrow, KEY_DownArrow, KEY_RightArrow, KEY_PageDown, KEY_None },
    /* ROW4 */ { KEY_None, KEY_None, KEY_None, KEY_None, KEY_None, KEY_Home },
...
void KEYBOARD_ChangeKey(uint8_t dst_key, uint8_t src_key)
{
    ...
    while(i)
    {
        --i;
        /* 交换src键和dst键 */
        if(*(memaddr + i) == src_key)
            *(memaddr + i) = dst_key;
        else if(*(memaddr + i) == dst_key)
            *(memaddr + i) = src_key;
    }
    DATAFLASH_Write_KeyArray();
}
...
UINT8 KEYBOARD_Custom_Function(void)
{
    ...
    if(g_keyboard_status.Fn == TRUE && KeyboardDat->Key2 == KEY_None)
    {   // 按下Fn+1个键或者只有Fn键
        last_state = 1;
        press_time = sys_time;
        if(KeyboardDat->Key1 == KEY_None && Fn_Mode == Fn_Mode_None)
        {   // 键盘布局复位
            Fn_Mode = Fn_Mode_Reset;
            Fn_cnt &= 0xF0;   // 清除其余计数器
        }
        else if(Fn_Mode == Fn_Mode_ChangeKey)
        {   // 设置改键 - 按Fn+src和Fn+dst
            if(Fn_cnt == 0x04)
                src_key = KeyboardDat->Key1;
            else if(Fn_cnt == 0x08)
                dst_key = KeyboardDat->Key1;
        }
        else if(KeyboardDat->Key1 == KEY_C && Fn_Mode != Fn_Mode_ChangeKey)
        {   // 设置改键 - 先按Fn+C
            Fn_Mode = Fn_Mode_ChangeKey;
            Fn_cnt &= 0x0C;
        }
        else if(KeyboardDat->Key1 == KEY_O && Fn_Mode != Fn_Mode_Enter_Cfg)
        {   // 配置参数模式
            Fn_Mode = Fn_Mode_Enter_Cfg;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_R && Fn_Mode != Fn_Mode_SoftReset)
        {   // 软件复位模式
            Fn_Mode = Fn_Mode_SoftReset;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_ESCAPE && Fn_Mode != Fn_Mode_RFJumptoBoot)
        {   // RF发送0x7A让接收器进BOOT
            Fn_Mode = Fn_Mode_RFJumptoBoot;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_B && Fn_Mode != Fn_Mode_JumpBoot)
        {   // 跳转BOOT模式
            Fn_Mode = Fn_Mode_JumpBoot;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_Subtraction && Fn_Mode != Fn_Mode_VolumeDown)
        {   // 音量减模式
        ...
        else if(KeyboardDat->Key1 == KEY_Equal && Fn_Mode != Fn_Mode_VolumeUp)
        {   // 音量加模式
        ...
        // else if(KeyboardDat->Key1 == KEY_Delete && Fn_Mode != Fn_Mode_PaintedEgg)
        // {   // 彩蛋模式
        //     Fn_Mode = Fn_Mode_PaintedEgg;
        //     Fn_cnt = 0;
        // }
        // else if(KeyboardDat->Key1 == KEY_T && Fn_Mode != Fn_Mode_DisEnableTP)
        // {   // 使能/失能小红点
        //     Fn_Mode = Fn_Mode_DisEnableTP;
        //     Fn_cnt = 0;
        // }
        else if(KeyboardDat->Key1 == KEY_F10 && Fn_Mode != Fn_Mode_USBMode)
        {   // 切换USB模式
            Fn_Mode = Fn_Mode_USBMode;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_F11 && Fn_Mode != Fn_Mode_BLEMode)
        {   // 切换蓝牙模式
            Fn_Mode = Fn_Mode_BLEMode;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_F12 && Fn_Mode != Fn_Mode_RFMode)
        {   // 切换RF模式
            Fn_Mode = Fn_Mode_RFMode;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_F9 && Fn_Mode != Fn_Mode_USBBLEMode)
        {   // 切换USB和BLE共存模式
            Fn_Mode = Fn_Mode_USBBLEMode;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_U && Fn_Mode != Fn_Mode_UDiskMode)
        {   // 开启U盘模式
            Fn_Mode = Fn_Mode_UDiskMode;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_G && Fn_Mode != Fn_Mode_GameMode)
        {   // 开启性能模式
            Fn_Mode = Fn_Mode_GameMode;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_PageUp && Fn_Mode != Fn_Mode_Select_USB_Send)
        {   // 共存模式下选择USB发送
            Fn_Mode = Fn_Mode_Select_USB_Send;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_PageDown && Fn_Mode != Fn_Mode_Select_BLE_Send)
        {   // 共存模式下选择BLE发送
            Fn_Mode = Fn_Mode_Select_BLE_Send;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_0 && Fn_Mode != Fn_Mode_BLE_ClearSNV)
        {   // 清除蓝牙SNV信息
            Fn_Mode = Fn_Mode_BLE_ClearSNV;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_1 && Fn_Mode != Fn_Mode_SelectDevice1)
        {   // 切换至设备1
            Fn_Mode = Fn_Mode_SelectDevice1;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_2 && Fn_Mode != Fn_Mode_SelectDevice2)
        {   // 切换至设备2
            Fn_Mode = Fn_Mode_SelectDevice2;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_3 && Fn_Mode != Fn_Mode_SelectDevice3)
        {   // 切换至设备3
            Fn_Mode = Fn_Mode_SelectDevice3;
            Fn_cnt = 0;
        }
        else if(KeyboardDat->Key1 == KEY_4 && Fn_Mode != Fn_Mode_SelectDevice4)
        {   // 切换至设备4
            Fn_Mode = Fn_Mode_SelectDevice4;
            Fn_cnt = 0;
        }
        // else if(KeyboardDat->Key1 == KEY_F1 && Fn_Mode != Fn_Mode_LED_Style1)
        // {   // 背光模式1
        //     Fn_Mode = Fn_Mode_LED_Style1;
        //     Fn_cnt = 0;
        // }
        // else if(KeyboardDat->Key1 == KEY_F2 && Fn_Mode != Fn_Mode_LED_Style2)
        // {   // 背光模式2
        //     Fn_Mode = Fn_Mode_LED_Style2;
        //     Fn_cnt = 0;
        // }
        // else if(KeyboardDat->Key1 == KEY_F3 && Fn_Mode != Fn_Mode_LED_Style3)
        // {   // 背光模式3
        //     Fn_Mode = Fn_Mode_LED_Style3;
        //     Fn_cnt = 0;
        // }
        // else if(KeyboardDat->Key1 == KEY_F4 && Fn_Mode != Fn_Mode_LED_Style4)
        // {   // 背光模式4
        //     Fn_Mode = Fn_Mode_LED_Style4;
        //     Fn_cnt = 0;
        // }
        // else if(KeyboardDat->Key1 == KEY_F5 && Fn_Mode != Fn_Mode_LED_Style5)
        // {   // 背光模式5
        //     Fn_Mode = Fn_Mode_LED_Style5;
        //     Fn_cnt = 0;
        // }
        // else if(KeyboardDat->Key1 == KEY_F6 && Fn_Mode != Fn_Mode_LED_Style6)
        // {   // 背光模式6
        ...
        {
            if(Fn_Mode == Fn_Mode_Reset)
                return 0;
            if(Fn_Mode != Fn_Mode_None && KeyboardDat->Key1 == KEY_None)
            {   // 先松开其他按键(后松开Fn)
    ...
    else if(last_state > 0 && g_keyboard_status.Fn == FALSE)
    {   // 松开Fn执行指令
    fn_do_operations:
        switch(Fn_Mode)
        {
            ...
            {   // 按下5次Fn重置按键
                ...
                else if(Fn_cnt == 0x20 && sys_time - press_time >= 3 * (1000 / SYS_PERIOD))
                {   // 第二下Fn长按进board boot --- 防止误擦除固件，该功能暂时屏蔽
                    //-APPJumpBoot();
                }
                break;
            }
            ...
            case Fn_Mode_SoftReset:
            {   // Fn+R软件复位
                Fn_Mode = Fn_Mode_None;
                SoftReset();
                break;
            }
            case Fn_Mode_RFJumptoBoot:
            {   // Fn+ESC发送0x7A让接收器进BOOT
                Fn_Mode = Fn_Mode_None;
            ...
            //     if(g_Enable_Status.tp == TRUE)
            //         OLED_UI_add_SHOW_ICONINFO_task(OLED_UI_ICON_TP_IDX, " ON");
            //     else
            //         OLED_UI_add_SHOW_ICONINFO_task(OLED_UI_ICON_TP_IDX, " OFF");
            //     break;
            // }
            case Fn_Mode_USBMode:
            {   // Fn+F10切换USB模式
                Fn_Mode = Fn_Mode_None;
            ...
            case Fn_Mode_SelectDevice1 ... Fn_Mode_SelectDevice4:
            {   // 按Fn+1~4切换设备
...
void KEYBOARD_Init(void)
{
    ...
    CustomKey[4][2] = KEY_Fn;                            // 保证上电Fn键在对应位置 - 其它键盘布局需修改此处
...
void KEYBOARD_Detection(void)
{
    ...
    if(CapsLock_cnt > 0)
        CapsLock_cnt--;
    if(g_capslock_status.press_Capslock_NormalKey && CapsLock_cnt == 0)
    {   // 短按CapsLock后，弹起CapsLock
    ...
    /* 基础功能 */
#if (ROW_SCAN_MODE)
    for(current_colum = 0; current_colum < COL_SIZE; current_colum++)
    {   // 查询哪一列改变
        if(KeyMatrix[current_row][current_colum] == 0 && Colum_GPIO_(ReadPortPin)(Colum_Pin[current_colum]) != 0)
        {   // 按下
#else
    for(current_row = 0; current_row < ROW_SIZE; current_row++)
    {   // 查询哪一行改变
        if(KeyMatrix[current_row][current_colum] == 0 && Row_GPIO_(ReadPortPin)(Row_Pin[current_row]) == 0)
        {   // 按下
#endif
            if(g_keyboard_status.SP_Key != 0)
                continue;                                // SP键生效后不接受其余按键
            KeyMatrix[current_row][current_colum] = 1;   // 矩阵状态变成按下
            // if(led_style_func == WS2812_Style_Touch && Key_To_LEDNumber[current_row][current_colum] != 0xFF)
            // {   // 触控呼吸灯模式
            ...
            if(KeyArr_Ptr[current_row][current_colum] == KEY_None)
            {   // 不做处理
                continue;
            }
            else if(KeyArr_Ptr[current_row][current_colum] == KEY_Fn)
            {   // 功能键
                g_keyboard_status.Fn = TRUE;
            }
            else if(KeyArr_Ptr[current_row][current_colum] >= KEY_SP_1)
            {   // SP键(单键复合)
            ...
            else if(KeyArr_Ptr[current_row][current_colum] >= KEY_MouseL)
            {   // 鼠标操作
                MouseDat->data[0] |= 1 << KeyArr_Ptr[current_row][current_colum] - KEY_MouseL;
                press_Normal_Key = TRUE;
                g_Ready_Status.keyboard_mouse_data = TRUE;   // 产生鼠标事件
            }
            else if(KeyArr_Ptr[current_row][current_colum] >= KEY_LeftCTRL)
            {   // Ctrl等特殊键
                press_Normal_Key = TRUE;
            ...
            {
                press_Normal_Key = TRUE;
                if(CustomKey[current_row][current_colum] == KEY_CapsLock)
                {   // CapsLock处理
        ...
        else if(KeyMatrix[current_row][current_colum] != 0 && Colum_GPIO_(ReadPortPin)(Colum_Pin[current_colum]) == 0)
        {   // 弹起
#else
        }
        else if(KeyMatrix[current_row][current_colum] != 0 && Row_GPIO_(ReadPortPin)(Row_Pin[current_row]) != 0)
        {   // 弹起
#endif
            KeyMatrix[current_row][current_colum] = 0;
            g_Ready_Status.keyboard_key_data = TRUE;   // 产生事件
            if(KeyArr_Ptr[current_row][current_colum] == KEY_Fn)
            {   // 功能键
            ...
            {
                if(CustomKey[current_row][current_colum] == KEY_CapsLock)
                {   // 弹起大小写键离开Extra_CustomKey层
                    KeyArr_Ptr = CustomKey;
                    g_capslock_status.press_Capslock = FALSE;
                    if(g_capslock_status.press_Capslock_with_other)
                    {   // 有使用其他层
                        g_capslock_status.press_Capslock_NormalKey = FALSE;
                        if(g_keyboard_status.SP_Key)
                        {   // 有使用SP键
                            g_keyboard_status.SP_Key = 0;
                            memset(KeyboardDat->data, 0, HID_KEYBOARD_DATA_LENGTH);
                            break;
                    ...
                    for(key_idx = 2; key_idx < HID_KEYBOARD_DATA_LENGTH; key_idx++)
                    {
                        if(KeyboardDat->data[key_idx] != KEY_None)
                        {
                            if((KeyboardDat->data[key_idx] == CustomKey[current_row][current_colum] ||
                                KeyboardDat->data[key_idx] == Extra_CustomKey[current_row][current_colum]))
                            {   // 弹起按键2层都清除
...
uint8_t KEYBOARD_EnterNumber(uint32_t *key, const char *preStr, const char *postStr, uint8_t limit_len)
{
    ...
    if(KeyboardDat->Key1 == KEY_BACKSPACE)
    {   // 退格单独处理
        if(idx > 0)
        {
            passkey_str[--idx] = '\0';
    ...
    else if(KeyboardDat->Key1 == KEY_ENTER && (idx == limit_len || limit_len == 0))
    {   // 最后一个按键是Enter则结束
    ...
    else if(KeyboardDat->Key1 == KEY_ESCAPE)
    {   // Esc清空输入项
        passkey = idx = passkey_str[0] = 0;
        OLED_PRINT("%s", preStr);
        return 0xFF;
