...
__attribute__((weak)) void HID_KEYBOARD_Process(void)
{
    ...
    if(g_Ready_Status.keyboard_key_data == TRUE)
    {   // 产生键盘数据
        ...
        else if(g_keyboard_status.enter_cfg == TRUE)
        {   // 处理界面配置
//            if(KEYBOARD_Custom_Function() && g_oled_smooth_end_flag == TRUE)
//            {   // 不带有Fn键且动画执行结束
...
//            }
        ...
        {
            if(KEYBOARD_Custom_Function())
            {   // 普通按键,带有Fn键处理信息则不产生键盘事件
                if(g_Ready_Status.keyboard_mouse_data == TRUE)
                {   // 发送键盘鼠标数据
...
tmosEvents HAL_ProcessEvent(tmosTaskID task_id, tmosEvents events)
{
    ...
    if(events & MAIN_CIRCULATION_EVENT)
    {
        ...
        else if(idle_cnt >= lp_max_period)
        {   // 进入低功耗模式
            idle_cnt = 0;
