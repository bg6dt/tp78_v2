    ...
    switch(*command_id)
    {
                ...
                case(uint8_t)VIA_ID_SWITCH_MATRIX_STATE:
                {
                    ...
                    for(i = 0; i < ROW_SIZE; i++)
                    {
                        ...
                        for(j = COL_SIZE - 1; j != 0xFF; j--)
                        {
                            temp = j % 8;
                            if(KeyMatrix[j][i])
                                command_data[k] |= 1 << temp;   // is pressed
                            if(temp == 0)
                                k++;
                        }
                    }
        ...
        **********************************/
        ...
        case(uint8_t)VIA_ID_DYNAMIC_KEYMAP_MACRO_SET_BUFFER:
        {
          ...
          command_data[2] = size
        **********************************/
            if(((command_data[0] << 8) | command_data[1]) == SP_KEY_NUMBER * (3 * 8 + 6 + 1) - 1 && command_data[2] == 0x1 && command_data[3] == 0xFF)
            {   // START flag
                sp_key_index = 0;
                sp_key_normal_key_index = 0;
                sp_key_next_direct_keycode_flag = FALSE;
